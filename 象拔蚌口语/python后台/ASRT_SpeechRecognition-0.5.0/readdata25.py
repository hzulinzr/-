...
class DataSpeech():
    ...
    def __init__(self, path, type, LoadToMem=False, MemWavCount=10000):
        ...
        self.dic_wavlist_aishell2 = {}
        self.dic_symbollist_aishell2 = {}
    ...
    def LoadDataList(self):
        ...
        # 设定选取哪一项作为要使用的数据集
        if (self.type == 'train'):
            filename_wavlist_aishell2 = 'aishell2' + self.slash + 'train.wav.txt'
            filename_symbollist_aishell2 = 'aishell2' + self.slash + 'train.syllable.txt'
        elif (self.type == 'dev'):
            filename_wavlist_aishell2 = 'aishell2' + self.slash + 'dev.wav.txt'
            filename_symbollist_aishell2 = 'aishell2' + self.slash + 'dev.syllable.txt'
        elif (self.type == 'test'):
            filename_wavlist_aishell2 = 'aishell2' + self.slash + 'test.wav.scp'
            filename_symbollist_aishell2 = 'aishell2' + self.slash + 'test.syllable.txt'

        else:
            filename_wavlist = ''  # 默认留空
            filename_symbollist = ''
        # 读取数据列表，wav文件列表和其对应的符号列表
        self.dic_wavlist_aishell2, self.list_wavnum_aishell2 = get_wav_list(self.datapath + filename_wavlist_aishell2)


        self.dic_symbollist_aishell2, self.list_symbolnum_aishell2 = get_wav_symbol(
            self.datapath + filename_symbollist_aishell2)
    ...
    def GetDataNum(self):
        ...
        '''
        num_wavlist_aishell2 = len(self.dic_wavlist_aishell2)
        num_symbollist_aishell2 = len(self.dic_symbollist_aishell2)

        print("num_wavlist_aishell2=" + str(num_wavlist_aishell2))
        print("num_symbollist_aishell2=" + str(num_symbollist_aishell2))

        if (num_wavlist_aishell2 == num_symbollist_aishell2):
            DataNum = num_wavlist_aishell2
        else:
            DataNum = -1
    ...
    def GetData(self, n_start, n_amount=1):
        ...
        length = len(self.list_wavnum_aishell2)
        filename = self.dic_wavlist_aishell2[self.list_wavnum_aishell2[n_start % length]]
        list_symbol = self.dic_symbollist_aishell2[self.list_symbolnum_aishell2[n_start % length]]
